...
# this is part of couchflow since template tags must be part
# of the INSTALLED_APPS tuple on settings.py
...
VERSION = ''
try:
    VERSION = subprocess.Popen(["git", "log", "-1", "--format=%h (%cd)",
        "--date=short"], stdout=subprocess.PIPE).communicate()[0].strip()
except os.error, e:
    print "OSError when fetching git revision: ", e
