...
function searchref_fix_table() {
  $("#selectWin").prependTo("#search_table_table_wrapper");
  $("#search-box-references").appendTo("#search_table .fg-toolbar:first");
}

function searchref_render(actions_function) {
  var config = {'extraData': ["filter", "reference", "item_type"], 'showImage': false};
  var table = render_search_table(actions_function, config);
}

function searchref_liveuse(update_function) {
  // remove all callback
  $('.liveuse').die();

  $('.liveuse').live('click', function(event){
      ...
      $.ajax({
           ...
           success: function(data){
               ...
               var data = data['data'];
               if(success != true){
                  alert('Campos Invalidos para el item "' + data.toString() + '"');
                  return;
               }
               if (!update_function) {
                  $.closePopupLayer();
